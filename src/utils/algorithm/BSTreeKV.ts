...
export default class BSTreeKV<K, V extends K> {
    ...
    minimum(): V | undefined {
      ...
      return (this.minimumAux(this.root) as BSTreeNode<V>).element;
    }
    ...
    maximum(): V | undefined {
      ...
      return (this.maximumAux(this.root) as BSTreeNode<V>).element;
    }
    ...
    private removeNode(node: BSTreeNode<V>): void {
      if (node.leftCh === null) {
        ...
        const y = this.minimumAux(node.rightCh) as BSTreeNode<V>;
        if (y.parent !== node) {
     ...
     * @private
     */
    private levelTraversalAux(node: BSTreeNode<V> | null, callback: util.ILoopFunction<V>): void {
     ...
     * @private
     */
    private preorderTraversalAux(node: BSTreeNode<V> | null, callback: util.ILoopFunction<V>, signal: { stop: boolean }): void {
      if (node === null || signal.stop) {
        return;
      }
      signal.stop = callback(node.element) === false;
     ...
     * @private
     */
    private postorderTraversalAux(node: BSTreeNode<V> | null, callback: util.ILoopFunction<V>, signal: { stop: boolean }): void {
      if (node === null || signal.stop) {
        return;
      }
